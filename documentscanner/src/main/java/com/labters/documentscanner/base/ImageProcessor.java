...
import com.labters.documentscanner.ApplyFiltersActivity;
...
public class ImageProcessor extends Handler {
    ...
    private final ApplyFiltersActivity mMainActivity;
    ...
    private int colorThresh = 120;        // threshold
    /////////TRIAL AND ERROR PART/////////

    public ImageProcessor(ApplyFiltersActivity mainActivity ) {
...
//            } else if ( command.equals("previewFrame")) {
...
//            }
    ...
    private boolean insideArea(Point[] rp, Size size) {
        ...
        return (
                rp[0].x <= leftPos && rp[0].y <= baseMeasure
    ...
    private void colorThresh(Mat src, int threshold) {
        ...
        for (int i=0; i < size; i+=3) {
            ...
            // avoid unneeded work
            if ( (double) (d[i] & 0xff) == 255 ) { continue; }

            double max = Math.max(Math.max(d[i] & 0xff, (double) (d[i + 1] & 0xff)), d[i + 2] & 0xff);
