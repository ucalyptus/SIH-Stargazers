...
app = FastAPI(
    title="Stargazers API",
    description="An Integrated API service for document validation for Stargazers App",
    version="1.0.1"
)
...
@app.post(STAGE_URL)
async def classify_house_stage(image: UploadFile = File(...)):
    try:
        contents = await image.read()
        image = Image.open(io.BytesIO(contents))
        resized_image = image.resize(
            (stage_input_width, stage_input_height), Image.ANTIALIAS)
...
@app.post(HVAL_URL)
async def validate_house(image: UploadFile = File(...)):
    try:
        contents = await image.read()
        image = Image.open(io.BytesIO(contents))
        resized_image = image.resize(
            (hval_input_width, hval_input_height), Image.ANTIALIAS)
...
async def image_segmentation(image: UploadFile = File(...)):
    try:
        ...
        resized_image = cropped_image.convert(
            'RGB').resize(seg_input_size, Image.BILINEAR)
        ...
        seg_interpreter.allocate_tensors()
        seg_interpreter.set_tensor(
            seg_input_details[0]['index'], image_for_prediction)
        seg_interpreter.invoke()
        # Retrieve the raw output map.
        raw_prediction = seg_interpreter.tensor(
            seg_interpreter.get_output_details()[0]['index'])()
